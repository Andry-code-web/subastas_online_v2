...
io.on("connection", (socket) => {
  ...
  socket.on("joinRoom", (room, userName) => {
    const query =
      "SELECT auctionEnded, currentWinner FROM subastas WHERE id = ?";
    conection.query(query, [room], (error, results) => {
      ...
      if (results.length > 0) {
        ...
        if (auctionEnded && currentWinner) {
          ...
          console.log(
            `Cliente se unió a una subasta ya terminada en la sala ${room}`
          );
        } else {
          socket.join(room);
          ...
          socket
            .to(room)
            .emit("participantJoined", { message: `${userName} se ha unido.` });
        }
      }
    });
  });
  ...
  socket.on("bid", (data) => {
    ...
    if (isNaN(bidValue) || bidValue <= auctions[room].currentBid) {
      console.log(
        "Puja inválida. Debe ser un número mayor que la puja actual."
      );
      return;
    }
    ...
    // Reiniciar el temporizador para todos los clientes
    io.to(room).emit("resetCountdown");

    const updateQuery =
      "UPDATE subastas SET currentWinner = ?, currentBid = ? WHERE id = ?";
    conection.query(updateQuery, [data.user, bidValue, room], (error) => {
      if (error) {
        ...
        console.log(
          `Ganador y puja actualizados en la subasta ${room}:`,
          data.user,
          bidValue
        );
      }
    ...
    conection.query(getUserIdQuery, [data.user], (error, results) => {
      ...
      if (results.length > 0) {
        ...
        console.error(
          "No se encontró el usuario con el nombre de usuario:",
          data.user
        );
      }
    });
  ...
  socket.on("endAuction", (room) => {
    if (!auctions[room].auctionEnded) {
      ...
      const updateQuery =
        "UPDATE subastas SET auctionEnded = true WHERE id = ?";
      conection.query(updateQuery, [room], (error) => {
        ...
        conection.query(getUserIdQuery, [username], (error, results) => {
          ...
          if (results.length > 0) {
            const userId = results[0].id;
            const updateOpportunitiesQuery =
        ...
        io.to(room).emit("auctionEnded", {
          winner: auctions[room].currentWinner,
        });
  ...
  // Añadir un nuevo evento para reiniciar el temporizador
  socket.on("resetCountdown", () => {
    io.to(auctionRoom).emit("resetCountdown");
  });
  ...
  setInterval(() => {
    Object.keys(lastHeartbeat).forEach((room) => {
      ...
      if (timeSinceLastHeartbeat > HEARTBEAT_TIMEOUT) {
        ...
        io.to(room).emit("clientDisconnected", {
          message: `Cliente desconectado de la subasta ${room}`,
        });
      }
    });
  }, 5000);
